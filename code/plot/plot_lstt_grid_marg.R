...
#for (i in 1:2500) {
    ...
    start_age = floor(stoch_bg_biome$x1[i])
    end_age = floor(stoch_bg_biome$x2[i])
...
for (i in 1:6) {
    ...
    titleij = bg_names[i]
    dij = d2[ d2$Area==i, ]
    pij = ggplot(data = dij, aes(x=age, y=count_norm, colour=Biome))
    pij = pij + geom_line(size=size, alpha=0.8)
    pij = pij + ylim(ylim)
   # pij = pij + scale_y_log10()
    pij = pij + scale_x_continuous("", trans="reverse", limits=c(max_time,0)) #, sec.axis = sec_axis(~ ., name=title_i, breaks=c() ))
    #pij = pij + ggtitle(titleij)
    pij = pij + annotate("text", x=max_time, y=ylim[2], label=titleij, colour="black", size=5, hjust=0, vjust=1)
    pij = pij + xlab("") + ylab("")
    pij = pij + scale_color_manual(values=biome_cols, labels = c("H", "L", "C", "T"), guide="legend")

    if (i==1) {
        pij_bg = pij + guides(alpha = F)
        p_bg_legend = get_legend(pij_bg)
    }
    pij = pij + theme(  plot.title = element_text(size=10),
...
pbiome=list()
for (i in 1:4) {
    titleij = biome_names[i]
    dij = d2[ d2$Biome==i, ]
    pij = ggplot(data = dij, aes(x=age, y=count_norm, colour=Area))
    pij = pij + geom_line(size=size, alpha=0.8)
    pij = pij + ylim(ylim)
    pij = pij + scale_x_continuous("", trans="reverse", limits=c(max_time,0)) #, sec.axis = sec_axis(~ ., name=title_i, breaks=c() ))
    #pij = pij + scale_y_log10()
    #pij = pij + ggtitle(titleij)
    #pij = pij + geom_text(x=max_time/2, y=ylim[2]*0.9, label=titleij)
    pij = pij + annotate("text", x=max_time, y=ylim[2], label=titleij, colour="black", size=5, vjust=1, hjust=0)
    pij = pij + xlab("") + ylab("")
    #pij = pij + scale_color_manual(values=biome_cols, labels = c("H", "L", "C", "T"), guide="legend")
    pij = pij + scale_color_manual(values=area_cols, labels = c("SEAs", "EAs", "Eur", "NAm", "CAm", "SAm"), guide="legend")

    if (i == 1) {
        px = pij
        
        pij_biome = pij + guides(alpha = F)
        p_biome_legend = get_legend(pij_biome)
        
        pij_freq = pij + scale_fill_manual(values=area_cols, labels = c("SEAs", "EAs", "Eur", "NAm", "LAm"), guide=FALSE)
        p_freq_legend = get_legend(pij_freq)
...
CairoPDF(  file=paste(plot_fp, "lstt/", base_fn, ".lstt_grid_marg.pdf", sep=""), width=7, height=7)
print(pboth)
dev.off()
