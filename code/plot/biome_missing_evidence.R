...
plot_fn = paste(plot_fp, "fig_temperate_root_sensitivity.pdf", sep="")
...
p = ggplot(df, aes(x=no_fossils, y=with_fossils))


f_wf_inv = approxfun( df$with_fossils, df$no_fossils )
f_wf = approxfun(  df$no_fossils, df$with_fossils )
...
p = p + geom_point( mapping=aes(x=0.25, y=f_wf(0.25)), col="black")
p = p + geom_point( mapping=aes(x=p_flip, y=f_wf(p_flip)), col="black")
p = p + geom_point( mapping=aes(x=p_sig, y=f_wf(p_sig)), col="black")
...
p = p + xlab("Posterior ( Fr. Root | Data_unk)")
p = p + ylab("Posterior ( Fr. Root | Data_obs, Data_unk)")

p = p + theme(panel.grid.major = element_blank(),
              panel.grid.minor = element_blank(),
              panel.background = element_blank(),
              axis.line = element_line(colour = "black"))
#p = p + scale_color_manual( name="Fossil taxa", labels=c("Ignore biomes", "Use biomes"), values=c("dodgerblue", "firebrick3") )
p
